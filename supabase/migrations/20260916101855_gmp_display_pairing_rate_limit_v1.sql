...
begin
  ...
  select * into v_row from public.gmp_display_pairing_rate_limits where key_hash = p_key_hash for update;
  if v_row.window_started_at <= p_now - interval '10 minutes' then
    update public.gmp_display_pairing_rate_limits set window_started_at=p_now,attempt_count=1,updated_at=p_now where key_hash=p_key_hash;
    return true;
  end if;
  ...
  update public.gmp_display_pairing_rate_limits set attempt_count=attempt_count+1,updated_at=p_now where key_hash=p_key_hash;
  return true;
end;
$$;
...
create index if not exists gmp_display_pairing_rate_limits_updated_idx on public.gmp_display_pairing_rate_limits(updated_at);
