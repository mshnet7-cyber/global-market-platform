-- Atomic commercial quotation creation.
-- Preserves the existing quote calculations while making header + lines one transaction.
...
begin
  ...
  -- Validate every referenced product inside the same transaction and tenant.
  ...
  values (
    ...
    p_valid_until,trim(p_currency),p_subtotal,greatest(p_discount_amount,0),
    greatest(p_vat_amount,0),greatest(p_total,0),nullif(trim(p_notes),''),(select auth.uid())
  )
