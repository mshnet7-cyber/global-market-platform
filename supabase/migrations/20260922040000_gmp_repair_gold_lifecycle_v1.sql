alter table public.gmp_repair_orders add column if not exists store_id uuid references public.gmp_stores(id);

create table if not exists public.gmp_repair_operation_idempotency (
 organization_id uuid not null references public.gmp_organizations(id) on delete cascade,
 client_ref text not null,
 repair_id uuid not null references public.gmp_repair_orders(id) on delete cascade,
...
alter table public.gmp_repair_operation_idempotency enable row level security;
drop policy if exists gmp_repair_operation_idempotency_member_read on public.gmp_repair_operation_idempotency;
create policy gmp_repair_operation_idempotency_member_read on public.gmp_repair_operation_idempotency
for select to authenticated using (
 exists(select 1 from public.gmp_organization_members m where m.organization_id=gmp_repair_operation_idempotency.organization_id and m.user_id=auth.uid())
);
...
begin
 ...
 if not exists(select 1 from public.gmp_organization_members m where m.organization_id=v_org and m.user_id=v_actor and m.role in ('owner','admin')) then
 ...
 select s.id into v_org from public.gmp_stores s
 where s.id=p_store_id and s.organization_id=v_repair.organization_id and (v_repair.branch_id is null or s.branch_id=v_repair.branch_id);
 ...
 if p_action='receive' then
   ...
   v_net:=greatest(v_repair.weight_received_grams,0);
   ...
   insert into public.gmp_repair_operation_idempotency(organization_id,client_ref,repair_id,action,response,created_by)
   values(v_repair.organization_id,trim(p_client_ref),p_repair_id,p_action,
     jsonb_build_object('repair_id',p_repair_id,'action',p_action,'status','in_repair','gold_ledger_entry_id',v_ledger_id),v_actor);
 ...
 if p_action='ready' then
   ...
   insert into public.gmp_repair_operation_idempotency(organization_id,client_ref,repair_id,action,response,created_by)
   values(v_repair.organization_id,trim(p_client_ref),p_repair_id,p_action,
     jsonb_build_object('repair_id',p_repair_id,'action',p_action,'status','ready'),v_actor);
 ...
 where a.organization_id=v_repair.organization_id and a.system_key=case p_payment_method when 'cash' then 'cash' when 'bank' then 'bank' when 'card' then 'card' when 'wallet' then 'wallet' else 'other' end and a.active limit 1;
 select a.id into v_sales from public.gmp_accounts a
 where a.organization_id=v_repair.organization_id and a.system_key='sales' and a.active limit 1;
 ...
 v_net:=greatest(coalesce(v_repair.weight_delivered_grams,v_repair.weight_received_grams),0);
 v_unit:=case when v_net>0 then coalesce(v_repair.amount,0)/v_net else 0 end;
 select public.gmp_post_gold_ledger_entry(
   v_repair.organization_id,v_repair.branch_id,p_store_id,null,'repair_out','out','repair',p_repair_id,
   'repair-deliver:'||p_repair_id::text,v_repair.karat::numeric,v_net,0,v_net,v_unit,coalesce(v_repair.amount,0),0,'OMR',
   jsonb_build_object('repair_no',v_repair.repair_no,'action','deliver')
 ) into v_ledger;
 v_ledger_id:=(v_ledger->>'entry_id')::uuid;
 ...
 values(v_repair.organization_id,v_repair.branch_id,'repair',p_repair_id,'إيراد إصلاح وتسليم',current_date,'posted',v_actor) returning id into v_journal;
 ...
 insert into public.gmp_repair_operation_idempotency(organization_id,client_ref,repair_id,action,response,created_by)
 values(v_repair.organization_id,trim(p_client_ref),p_repair_id,p_action,
   jsonb_build_object('repair_id',p_repair_id,'action',p_action,'status','delivered','gold_ledger_entry_id',v_ledger_id,'journal_entry_id',v_journal),v_actor);
