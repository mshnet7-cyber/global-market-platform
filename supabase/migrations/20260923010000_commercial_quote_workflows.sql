-- Commercial workflow hardening: transactional quotation lifecycle and conversion.
...
begin
  ...
  update public.gmp_sales_quotes set status=p_status, updated_at=now() where id=q.id;
...
begin
  ...
  if q.status='converted' then
    return jsonb_build_object('quote_id',q.id,'status','converted','sale_id',
      (select reference_id from public.gmp_journal_entries where organization_id=p_organization_id and reference_type='quote_conversion' and reference_id=q.id limit 1));
  end if;
  ...
  ) order by l.id),'[]'::jsonb) into lines
  from public.gmp_sales_quote_lines l where l.quote_id=q.id;
