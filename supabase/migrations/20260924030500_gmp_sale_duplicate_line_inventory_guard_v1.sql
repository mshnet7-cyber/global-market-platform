-- Fix duplicate sale-line inventory validation.
-- The sale RPC previously validated inventory one line at a time, allowing
-- repeated product_id entries whose combined demand exceeded stock.
-- Keep the existing business contract; aggregate demand per product before posting.
...
begin
  ...
  for v_product, v_qty, v_weight in
    select (value->>'product_id')::uuid,
           sum(nullif(value->>'quantity','')::numeric),
           sum(coalesce(nullif(value->>'weight_grams','')::numeric,0))
    from jsonb_array_elements(p_lines)
    group by (value->>'product_id')
    order by (value->>'product_id')
  loop
    select current_quantity,current_weight_grams,cost_price into v_available,v_available_weight,v_unit_cost
    from public.gmp_products where id=v_product and store_id=p_store_id and active=true for update;
    if not found then raise exception 'product not found'; end if;
    if v_available<v_qty or v_available_weight<v_weight then raise exception 'insufficient inventory'; end if;
  end loop;

  for v_line in select value from jsonb_array_elements(p_lines) order by (value->>'product_id') loop
    v_qty:=nullif(v_line->>'quantity','')::numeric; v_unit_price:=coalesce(nullif(v_line->>'unit_price','')::numeric,0); v_making:=coalesce(nullif(v_line->>'making_charge','')::numeric,0); v_discount:=coalesce(nullif(v_line->>'discount_amount','')::numeric,0); v_line_vat:=coalesce(nullif(v_line->>'vat_amount','')::numeric,0);
...
REVOKE EXECUTE ON FUNCTION public.gmp_create_and_post_sale(uuid,uuid,uuid,uuid,text,text,jsonb) FROM anon, public;
