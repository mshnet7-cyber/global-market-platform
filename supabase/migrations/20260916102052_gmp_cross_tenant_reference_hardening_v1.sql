-- Production migration 20260916102052.
-- Hardens purchase, manual-journal, and sale RPCs against cross-tenant references.
...
declare v_actor uuid := (select auth.uid()); v_purchase_id uuid; v_line jsonb; v_qty numeric; v_weight numeric; v_unit_cost numeric; v_vat numeric; v_making numeric; v_subtotal numeric:=0; v_vat_total numeric:=0; v_branch uuid; v_product uuid;
begin
 ...
 for v_line in select value from jsonb_array_elements(p_lines) loop
  ...
  if v_product is not null and not exists(select 1 from public.gmp_products p join public.gmp_stores s on s.id=p.store_id where p.id=v_product and s.organization_id=p_organization_id and s.id=p_store_id and p.active=true) then raise exception 'product not found'; end if;
  v_qty:=nullif(v_line->>'quantity','')::numeric; v_weight:=coalesce(nullif(v_line->>'weight_grams','')::numeric,0); v_unit_cost:=nullif(v_line->>'unit_cost','')::numeric; v_vat:=coalesce(nullif(v_line->>'vat_amount','')::numeric,0); v_making:=coalesce(nullif(v_line->>'making_charge','')::numeric,0);
  ...
  v_subtotal:=v_subtotal+(v_qty*v_unit_cost)+v_making; v_vat_total:=v_vat_total+v_vat;
 end loop;
 insert into public.gmp_purchases(organization_id,branch_id,store_id,supplier_id,invoice_no,status,subtotal,vat_amount,total,created_by) values(p_organization_id,coalesce(p_branch_id,v_branch),p_store_id,p_supplier_id,nullif(trim(p_invoice_no),''),'draft',v_subtotal,v_vat_total,v_subtotal+v_vat_total,v_actor) returning id into v_purchase_id;
 for v_line in select value from jsonb_array_elements(p_lines) loop
  insert into public.gmp_purchase_lines(purchase_id,product_id,raw_description,sku,barcode,karat,country_of_origin,quantity,weight_grams,unit_cost,making_charge,vat_amount) values(v_purchase_id,nullif(v_line->>'product_id','')::uuid,left(v_line->>'raw_description',500),nullif(left(v_line->>'sku',80),''),nullif(left(v_line->>'barcode',80),''),nullif(left(v_line->>'karat',20),''),nullif(left(v_line->>'country_of_origin',80),''),nullif(v_line->>'quantity','')::numeric,coalesce(nullif(v_line->>'weight_grams','')::numeric,0),nullif(v_line->>'unit_cost','')::numeric,coalesce(nullif(v_line->>'making_charge','')::numeric,0),coalesce(nullif(v_line->>'vat_amount','')::numeric,0));
 end loop;
...
begin
 ...
 for v_line in select value from jsonb_array_elements(p_lines) loop insert into public.gmp_journal_lines(journal_entry_id,account_id,debit,credit,memo) values(v_entry_id,(v_line->>'account_id')::uuid,coalesce(nullif(v_line->>'debit','')::numeric,0),coalesce(nullif(v_line->>'credit','')::numeric,0),nullif(trim(v_line->>'memo'),'') ); end loop;
...
returns jsonb language plpgsql set search_path to 'public' as $$
declare v_actor uuid := (select auth.uid()); v_sale_id uuid; v_invoice_no bigint; v_journal_id uuid; v_payment_account uuid; v_inventory uuid; v_sales uuid; v_cogs uuid; v_vat uuid; v_cash uuid; v_bank uuid; v_card uuid; v_wallet uuid; v_other uuid; v_branch uuid; v_line jsonb; v_product uuid; v_qty numeric; v_weight numeric; v_unit_price numeric; v_making numeric; v_discount numeric; v_line_vat numeric; v_line_total numeric; v_subtotal numeric:=0; v_vat_total numeric:=0; v_cogs_total numeric:=0; v_unit_cost numeric; v_available numeric; v_available_weight numeric;
begin
 if v_actor is null then raise exception 'authentication required'; end if;
