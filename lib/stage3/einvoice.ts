...
export function getEInvoiceStatus() {
  const c = cfg();
  const state: IntegrationState = c.baseUrl && c.apiKey ? "live" : "integration_ready";
  return { state, provider: c.provider, reason: state === "live" ? undefined : "government_or_provider_credentials_not_configured" };
}
...
export function validateInvoicePayload(payload: Record<string, unknown>) {
  ...
  if (missing.length) throw new Error(`invoice_payload_missing_${missing.join("_")}`);
...
export async function submitInvoice(payload: Record<string, unknown>) {
  const c = cfg();
  if (!c.baseUrl || !c.apiKey) throw new Error("einvoice_not_configured");
  const requestHash = createHash("sha256").update(JSON.stringify(payload)).digest("hex");
  const response = await fetch(`${c.baseUrl}/invoices`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      authorization: `Bearer ${c.apiKey}`,
      "x-gmp-request-hash": requestHash,
    },
