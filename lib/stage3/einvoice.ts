...
export async function submitInvoice(payload: Record<string,unknown>) {
  ...
  const response=await fetch(`${c.baseUrl}/invoices`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${c.apiKey}`,"x-gmp-request-hash":requestHash},body:JSON.stringify(payload),cache:"no-store"});
