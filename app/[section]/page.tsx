...
export default async function SectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams?: Promise<{ country?: string; language?: string }> }) {
  ...
  return <div className={`page-frame section-page section-${section}`} dir={isRtl ? "rtl" : "ltr"} lang={language}>
    <header className="topbar"><div className="container nav site-nav"><Link href="/" className="brand" aria-label="Global Market" aria-current="page"><span className="brand-mark">GM</span><span>GLOBAL <b>MARKET</b></span></Link><nav className="nav-links" aria-label={language === "ar" ? "التنقل الرئيسي" : "Primary"}><Link href={`/gold?country=${country.code}&language=${language}`} aria-current={section === "gold" ? "page" : undefined}>{messages.gold}</Link><Link href={`/silver?country=${country.code}&language=${language}`} aria-current={section === "silver" ? "page" : undefined}>{messages.silver}</Link><Link href={`/markets?country=${country.code}&language=${language}`} aria-current={section === "markets" ? "page" : undefined}>{messages.markets}</Link><Link href={`/stocks?country=${country.code}&language=${language}`} aria-current={section === "stocks" ? "page" : undefined}>{messages.stocks}</Link><Link href={`/currencies?country=${country.code}&language=${language}`} aria-current={section === "currencies" ? "page" : undefined}>{messages.currencies}</Link><Link href={`/news?country=${country.code}&language=${language}`} aria-current={section === "news" ? "page" : undefined}>{messages.news}</Link></nav><details className="mobile-site-menu"><summary aria-label={language === "ar" ? "فتح قائمة التنقل" : "Open navigation"}>☰</summary><nav aria-label={language === "ar" ? "التنقل السريع" : "Quick navigation"}><Link href={`/gold?country=${country.code}&language=${language}`}>{messages.gold}</Link><Link href={`/silver?country=${country.code}&language=${language}`}>{messages.silver}</Link><Link href={`/markets?country=${country.code}&language=${language}`}>{messages.markets}</Link><Link href={`/stocks?country=${country.code}&language=${language}`}>{messages.stocks}</Link><Link href={`/currencies?country=${country.code}&language=${language}`}>{messages.currencies}</Link><Link href={`/news?country=${country.code}&language=${language}`}>{messages.news}</Link><Link href="/pricing">{language === "ar" ? "الباقات" : "Plans"}</Link><Link href="/directory">{language === "ar" ? "المحلات" : "Stores"}</Link><Link href="/marketplace">{messages.marketplace}</Link><Link href="/login">{messages.login}</Link></nav></details><div className="nav-actions"><DisplayPreferences language={language as any} initialTheme={initialTheme} initialFontScale={initialFontScale} /><div className="nav-preferences"><span className="country-dot" />{countryName}</div><Link href="/login" className="btn btn-ghost">{messages.login}</Link><Link href="/demo" className="btn btn-primary">{messages.createDisplay}</Link></div></div></header>
    <main className="container site-main section-shell"><section className="page-hero"><div className="section-header"><div><div className="eyebrow"><span className="live-dot" />{section.toUpperCase()} · {countryName}</div><h1>{title}</h1></div><div className="meta-block">{sectionDescription}<br />{messages.referenceOnly}{(section === "markets" || section === "stocks") && <><br />{messages.marketDataNote}</>}</div></div></section>
