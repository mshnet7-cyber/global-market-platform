...
export default async function DashboardPage() {
  ...
  return <div className="dashboard-shell">
    ...
    <main className="container dashboard-main">
      ...
      <section className="card subscription-panel"><div className="section-head"><div><div className="eyebrow">الاشتراك</div><h2>اشتراكك الحالي</h2></div><span className="status">{currentPlan?.name ?? "غير محدد"}</span></div><div className="plan-strip"><div className="plan-tile highlight"><div className="meta">الباقة الحالية</div><div className="plan-price">{currentPlan?.name ?? "—"}</div><div className="meta">{subscription?.current_period_end ? `تنتهي ${new Date(subscription.current_period_end).toLocaleDateString("ar-OM")}` : "لا يوجد تاريخ انتهاء مسجل"}</div></div>{plans.map((plan) => <div className="plan-tile" key={plan.code}><div className="meta">{plan.name}</div><div className="plan-price"><MoneyDisplay value={plan.monthly} currency="OMR" locale="ar-OM" maximumFractionDigits={0} /></div><div className="meta">شهريًا · خصم الفروع {plan.discount}</div></div>)}</div><div className="actions" style={{marginTop:16}}><Link href="/pricing" className="btn primary">عرض الباقات</Link><span className="meta">الشاشة الإضافية: <MoneyDisplay value={4} currency="OMR" locale="ar-OM" maximumFractionDigits={0} /> شهريًا · <MoneyDisplay value={21} currency="OMR" locale="ar-OM" maximumFractionDigits={0} /> لـ6 أشهر · <MoneyDisplay value={44} currency="OMR" locale="ar-OM" maximumFractionDigits={0} /> سنويًا.</span></div></section>
