...
import { ADDITIONAL_SCREEN_PRICING, MERCHANT_PLANS, getMerchantPlan } from "../../lib/saas-plans";
...
export default async function DashboardPage() {
  ...
  const currentPlan = getMerchantPlan(planRelation?.code ?? planCode);
  ...
  return <div className="dashboard-shell">
    ...
    <main className="container dashboard-main">
      ...
      <section className="section"><div className="section-head"><div><div className="eyebrow">مساحة العمل</div><h2>الوحدات الأساسية</h2></div><span className="meta">اختر العملية التي تريد تنفيذها</span></div><div className="grid grid-4">{modules.map(([href,title,description], index) => <Link href={href} className="card module-card" key={href}><div className="module-icon">{String(index + 1).padStart(2, "0")}</div><strong>{title}</strong><div className="meta">{description}</div><span className="module-arrow">←</span></Link>)}</div></section>
      <section className="section"><div className="section-head"><div><div className="eyebrow">مركز التحكم</div><h2>الخدمات والإدارة</h2></div></div><div className="grid four">{quickLinks.map(([href,title,description,icon]) => <Link href={href === "/dashboard/api-keys" && !["pro","business"].includes(planCode || "") ? "/pricing" : (["/dashboard/invoicing","/dashboard/cameras","/dashboard/compliance"].includes(href) && !business ? "/pricing" : href)} className="card module-card" key={href}><div className="module-icon">{icon}</div><strong>{title}</strong><div className="meta">{((href === "/dashboard/api-keys" && !["pro","business"].includes(planCode || "")) || (["/dashboard/invoicing","/dashboard/cameras","/dashboard/compliance"].includes(href) && !business)) ? "متاح بحسب الباقة والصلاحيات." : description}</div><span className="module-arrow">←</span></Link>)}</div></section>
      <section className="card subscription-panel"><div className="section-head"><div><div className="eyebrow">الاشتراك</div><h2>اشتراكك الحالي</h2></div><span className="status">{currentPlan?.name ?? "غير محدد"}</span></div><div className="plan-strip"><div className="plan-tile highlight"><div className="meta">الباقة الحالية</div><div className="plan-price">{currentPlan?.name ?? "—"}</div><div className="meta">{subscription?.current_period_end ? `تنتهي ${new Date(subscription.current_period_end).toLocaleDateString("ar-OM")}` : "لا يوجد تاريخ انتهاء مسجل"}</div></div>{MERCHANT_PLANS.map((plan) => <div className="plan-tile" key={plan.code}><div className="meta">{plan.name}</div><div className="plan-price"><MoneyDisplay value={plan.monthly} currency="OMR" locale="ar-OM" maximumFractionDigits={0} /></div><div className="meta">شهريًا · خصم الفروع {plan.extraBranchDiscount}</div></div>)}</div><div className="actions" style={{marginTop:16}}><Link href="/pricing" className="btn primary">عرض الباقات</Link><span className="meta">الشاشة الإضافية: <MoneyDisplay value={ADDITIONAL_SCREEN_PRICING.monthly} currency="OMR" locale="ar-OM" maximumFractionDigits={0} /> شهريًا · <MoneyDisplay value={ADDITIONAL_SCREEN_PRICING.halfYear} currency="OMR" locale="ar-OM" maximumFractionDigits={0} /> لـ6 أشهر · <MoneyDisplay value={ADDITIONAL_SCREEN_PRICING.yearly} currency="OMR" locale="ar-OM" maximumFractionDigits={0} /> سنويًا.</span></div></section>
      <section className="section"><div className="section-head"><div><div className="eyebrow">المتاجر</div><h2>المتاجر والفروع</h2></div><span className="status">{stores?.length ?? 0}</span></div>{stores?.length ? <div className="grid three">{stores.map((store) => <Link href={`/store/${store.slug}`} className="card" key={store.id}><div className="card-top"><strong>{store.name}</strong><span className="status">{store.country_code}</span></div><div className="meta store-meta">{store.currency} · {store.timezone}</div></Link>)}</div> : <div className="empty-state">لم يتم إنشاء متجر بعد. أضف أول متجر من النموذج أدناه.</div>}{role !== "viewer" && <form action="/api/stores/create" method="post" className="card two-col create-store-form"><label className="label">اسم المتجر<input className="select" name="name" placeholder="اسم المتجر" minLength={2} maxLength={120} required /></label><label className="label">الدولة<select className="select" name="country_code" defaultValue={appConfig.defaultCountry}>{countries.map((c) => <option key={c.code} value={c.code}>{c.name} ({c.code})</option>)}</select></label><label className="label">العملة<input className="select" name="currency" defaultValue={appConfig.defaultCurrency} maxLength={3} required /></label><label className="label">المنطقة الزمنية<input className="select" name="timezone" defaultValue="Asia/Muscat" required /></label><div className="actions form-actions"><button className="btn primary" type="submit">إنشاء متجر</button></div></form>}</section>
    </main>
  </div>;
}
