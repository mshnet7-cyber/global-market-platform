...
export default async function DashboardPage() {
  ...
  return <div className="dashboard-shell">
    ...
    <main className="container dashboard-main">
      ...
      <section className="section"><div className="section-head"><div><div className="eyebrow">CONTROL CENTER</div><h2>الخدمات والإدارة</h2></div></div><div className="grid three">{quickLinks.map(([href,title,description,icon]) => <Link href={(href === "/dashboard/api-keys" && !["pro","business"].includes(planCode || "")) ? "/pricing" : ((href === "/dashboard/integrations" || href === "/display" || href === "/dashboard/invoicing") && !business) ? "/pricing" : href} className="card module-card" key={href}><div className="module-icon">{icon}</div><strong>{title}</strong><div className="meta">{((href === "/dashboard/api-keys" && !["pro","business"].includes(planCode || "")) || ((href === "/display" || href === "/dashboard/invoicing" || href === "/dashboard/integrations") && !business)) ? "متاح بحسب الباقة والصلاحيات." : description}</div><span className="module-arrow">←</span></Link>)}</div></section>
