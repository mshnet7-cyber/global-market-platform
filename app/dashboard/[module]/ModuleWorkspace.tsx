...
function AccountField({name,label,accounts}:{name:string;label:string;accounts:Row[]}){return <label className="field"><span>{label}</span><select name={name} required defaultValue=""><option value="" disabled>اختر الحساب</option>{accounts.map(a=><option key={String(a.id)} value={String(a.id)}>{String(a.code)} — {String(a.name)}</option>)}</select></label>}
...
export default function ModuleWorkspace({module,stores}:{module:string;stores:StoreOption[]}){
  ...
  const form=()=> <form className="card" onSubmit={submit}>
    {module==="expenses"&&<><Field name="category" label="التصنيف" required/><AccountField name="expense_account_id" label="حساب المصروف" accounts={accounts}/><AccountField name="payment_account_id" label="حساب الدفع" accounts={accounts}/><Field name="amount" label="المبلغ قبل الضريبة" type="number" min="0.001" required/><Field name="vat_amount" label="ضريبة القيمة المضافة" type="number" min="0"/><Field name="expense_date" label="التاريخ" type="date"/><Field name="description" label="الوصف"/></>}
    ...
    {module==="accounting"&&<><AccountField name="debit_account" label="الحساب المدين" accounts={accounts}/><AccountField name="credit_account" label="الحساب الدائن" accounts={accounts}/><Field name="amount" label="المبلغ" type="number" min="0.001" required/><Field name="description" label="البيان" required/><Field name="entry_date" label="التاريخ" type="date"/></>}
    <button className="btn primary" disabled={busy||((module==="inventory"||module==="purchases")&&stores.length===0)||(module==="accounting"&&accounts.length<2)}>{busy?"جاري الحفظ…":"حفظ"}</button>{message&&<div className="meta" style={{marginTop:10}} role="status">{message}</div>}{module==="accounting"&&accounts.length<2&&<div className="notice" role="status">لا يمكن إنشاء قيد قبل توفر حسابين نشطين على الأقل.</div>}
  </form>;
  return <>
