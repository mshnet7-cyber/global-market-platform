...
export async function POST(request:Request){
 ...
 try{
  ...
  if(action==="quote"){
   ...
   const {data:quote,error}=await access.supabase.from("gmp_sales_quotes").insert({organization_id:access.organization.id,store_id,branch_id:uuid(txt(b.branch_id,80))?txt(b.branch_id,80):null,customer_id:uuid(txt(b.customer_id,80))?txt(b.customer_id,80):null,quote_no:no,status:"draft",valid_until:txt(b.valid_until,20)||null,currency:txt(b.currency,8)||"OMR",subtotal,discount_amount:discount,vat_amount:vat,total,notes:txt(b.notes,1500)||null,created_by:access.user.id}).select("*").single();
