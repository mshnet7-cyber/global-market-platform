...
export async function POST(request: Request) {
  ...
  const { error: memberError } = await supabase.from("gmp_organization_members").insert({ organization_id: org.id, user_id: data.user.id, role: "owner" });
  ...
  const { error: storeError } = await supabase.from("gmp_stores").insert({ organization_id: org.id, name, slug: storeSlug, country_code: "OM", currency: "OMR", timezone: "Asia/Muscat" });
