...
export async function POST(request: Request) {
  ...
  if (!data || data.revoked_at || (data.expires_at && new Date(data.expires_at).getTime() <= Date.now())) return NextResponse.json({ ok: false, error: "session_revoked_or_expired" }, { status: 401, headers: { "cache-control": "no-store" } });
  ...
  if (error) return NextResponse.json({ ok: false, error: "heartbeat_failed" }, { status: 500, headers: { "cache-control": "no-store" } });
  return NextResponse.json({ ok: true, screen_id: data.screen_id, server_time: now }, { headers: { "cache-control": "no-store" } });
}
