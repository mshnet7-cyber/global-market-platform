...
export async function POST(request: Request) {
  ...
  const { data: store } = await supabase.from("gmp_stores").select("organization_id").eq("id", screen.store_id).maybeSingle();
  ...
  await admin.from("gmp_screen_sessions").update({ revoked_at: now }).eq("screen_id", screenId).is("revoked_at", null);
