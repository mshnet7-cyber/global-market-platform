...
export async function POST(request: Request) {
  ...
  const { data: subscription } = await admin.from("gmp_subscriptions").select("plan_id,status").eq("organization_id", org.id).maybeSingle();
  ...
  if (subscription?.plan_id) {
    ...
    if (entitlement?.max_screens) {
      const { count } = await admin.from("gmp_screens").select("id", { count: "exact", head: true }).in("store_id", [storeId]);
