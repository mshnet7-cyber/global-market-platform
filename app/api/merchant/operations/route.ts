...
export async function GET(request: Request) {
  try {
    const url = new URL(request.url); const module = url.searchParams.get("module") ?? ""; const access = plans[module];
...
export async function POST(request: Request) {
  try {
    const body=await request.json(); const module=String(body.module??""); const access=plans[module]; if(!access)return NextResponse.json({error:"invalid_module"},{status:404});
    ...
    if(module==="purchases"){
      ...
      let subtotal=0,vat=0; const normalized=[]; for(const raw of lines){const quantity=num(raw.quantity),unitCost=num(raw.unit_cost),lineVat=num(raw.vat_amount??0),weight=num(raw.weight_grams??0); if(!raw.raw_description||quantity===null||quantity<=0||unitCost===null||unitCost<0||lineVat===null||lineVat<0||weight===null||weight<0)return NextResponse.json({error:"invalid_purchase_line"},{status:400}); subtotal+=quantity*unitCost;vat+=lineVat;normalized.push({product_id:raw.product_id||null,raw_description:String(raw.raw_description).slice(0,500),sku:raw.sku?String(raw.sku).slice(0,80):null,barcode:raw.barcode?String(raw.barcode).slice(0,80):null,karat:raw.karat?String(raw.karat).slice(0,20):null,country_of_origin:raw.country_of_origin?String(raw.country_of_origin).slice(0,80):null,quantity,weight_grams:weight,unit_cost:unitCost,making_charge:num(raw.making_charge??0)??0,vat_amount:lineVat});}
      const {data:purchase,error}=await supabase.from("gmp_purchases").insert({organization_id:organization.id,branch_id:body.branch_id||null,store_id:storeId,supplier_id:body.supplier_id||null,invoice_no:body.invoice_no?String(body.invoice_no).slice(0,80):null,status:"draft",subtotal,vat_amount:vat,total:subtotal+vat,created_by:user.id}).select("*").single(); if(error||!purchase)return NextResponse.json({error:error?.message??"purchase_create_failed"},{status:400});
