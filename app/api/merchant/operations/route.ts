...
import { requireMerchantPlan, requireMerchantEntitlement } from "../../../../lib/merchant-access";
...
const entitlements: Record<string, Parameters<typeof requireMerchantEntitlement>[0]> = { purchases: "pos", expenses: "accounting", repairs: "repairs", "buy-gold": "person_gold_purchase", inventory: "inventory", accounting: "accounting", tax: "tax_reports" };
...
async function validateBranch(supabase: Awaited<ReturnType<typeof requireMerchantPlan>>["supabase"], organizationId: string, branchId: unknown) {
...
async function validateCustomer(supabase: Awaited<ReturnType<typeof requireMerchantPlan>>["supabase"], organizationId: string, customerId: unknown) {
  if (!customerId) return true;
  const { data } = await supabase.from("gmp_customers").select("id").eq("id", String(customerId)).eq("organization_id", organizationId).maybeSingle();
  return Boolean(data);
}
...
export async function GET(request: Request) {
  try {
    ...
    const { supabase, organization } = await requireMerchantEntitlement(entitlements[moduleName]); const limit = Math.min(Math.max(Number(url.searchParams.get("limit") ?? 20) || 20, 1), 100);
  ...
  } catch(error){const message=error instanceof Error?error.message:"unexpected_error";return NextResponse.json({error:message},{status:message==="merchant_entitlement_required"?403:500});}
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const {supabase,user,organization}=await requireMerchantEntitlement(entitlements[moduleName]);
    ...
    if(moduleName==="repairs"){
      if(body.action==="status"){
        ...
        const { data: current } = await supabase.from("gmp_repair_orders").select("status").eq("id",id).eq("organization_id",organization.id).maybeSingle();
        if (!current) return NextResponse.json({error:"repair_not_found"},{status:404});
      ...
      if (!(await validateBranch(supabase, organization.id, body.branch_id))) return NextResponse.json({error:"branch_not_found"},{status:400});
      if (!(await validateCustomer(supabase, organization.id, body.customer_id))) return NextResponse.json({error:"customer_not_found"},{status:400});
      const {data,error}=await supabase.from("gmp_repair_orders").insert({organization_id:organization.id,branch_id:body.branch_id||null,customer_id:body.customer_id||null,item_description:description.slice(0,500),metal:body.metal?String(body.metal).slice(0,40):null,karat:body.karat?String(body.karat).slice(0,20):null,weight_received_grams:weight,damage_description:body.damage_description?String(body.damage_description).slice(0,1000):null,repair_type:body.repair_type?String(body.repair_type).slice(0,120):null,expected_days:expectedDays,amount,notes:body.notes?String(body.notes).slice(0,1000):null,created_by:user.id}).select("*").single(); if(error)return NextResponse.json({error:error.message},{status:400}); return NextResponse.json({success:true,row:data},{status:201});
    }
    if(moduleName==="buy-gold"){
      const name=String(body.seller_name??"").trim(),phone=String(body.seller_phone??"").trim(),identity=String(body.identity_document_path??"").trim(),weight=num(body.weight_grams),price=num(body.purchase_price),paymentMethod=String(body.payment_method??"cash"); if(!name||!phone||!identity||weight===null||weight<=0||price===null||price<=0||!methods.has(paymentMethod))return NextResponse.json({error:"invalid_person_gold_purchase"},{status:400});
      if (!(await validateBranch(supabase, organization.id, body.branch_id))) return NextResponse.json({error:"branch_not_found"},{status:400});
      const {data,error}=await supabase.from("gmp_person_gold_purchases").insert({organization_id:organization.id,branch_id:body.branch_id||null,seller_name:name.slice(0,200),seller_phone:phone.slice(0,40),identity_document_path:identity.slice(0,1000),item_description:body.item_description?String(body.item_description).slice(0,500):null,karat:body.karat?String(body.karat).slice(0,20):null,weight_grams:weight,market_reference_price:num(body.market_reference_price),purchase_price:price,payment_method:paymentMethod,created_by:user.id}).select("*").single(); if(error)return NextResponse.json({error:error.message},{status:400}); return NextResponse.json({success:true,row:data},{status:201});
    }
    if(moduleName==="inventory"){
      if(body.action==="create"){
        const name=String(body.name??"").trim(),storeId=String(body.store_id??""),price=num(body.price??0),costPrice=num(body.cost_price??0),makingCharge=num(body.making_charge??0),initialQuantity=num(body.current_quantity??0),initialWeight=num(body.current_weight_grams??0); if(!name||!storeId||price===null||price<0||costPrice===null||costPrice<0||makingCharge===null||makingCharge<0||initialQuantity===null||initialQuantity<0||initialWeight===null||initialWeight<0)return NextResponse.json({error:"invalid_product"},{status:400});
        const {data,error}=await supabase.rpc("gmp_create_inventory_product",{p_organization_id:organization.id,p_store_id:storeId,p_name:name,p_sku:body.sku?String(body.sku).slice(0,80):null,p_barcode:body.barcode?String(body.barcode).slice(0,80):null,p_category:body.category?String(body.category).slice(0,100):null,p_karat:body.karat?String(body.karat).slice(0,20):null,p_price:price,p_cost_price:costPrice,p_making_charge:makingCharge,p_initial_quantity:initialQuantity,p_initial_weight:initialWeight});
    ...
    if(moduleName==="purchases"){
      const storeId=String(body.store_id??""),lines=Array.isArray(body.lines)?body.lines:[]; if(!storeId||!lines.length||lines.length>MAX_LINES)return NextResponse.json({error:lines.length>MAX_LINES?"too_many_purchase_lines":"store_and_lines_required"},{status:400});
    ...
    if(moduleName==="accounting"){
      const lines=Array.isArray(body.lines)?body.lines:[]; const description=String(body.description??"").trim(); if(!description||lines.length<2||lines.length>MAX_LINES)return NextResponse.json({error:lines.length>MAX_LINES?"too_many_journal_lines":"journal_required"},{status:400});
