...
async function run(request:Request){
  ...
  if(error) return NextResponse.json({error:"claim_failed"},{status:503});
  let processed=0,delivered=0;
  for(const row of data??[]){
    processed++;
    if(await deliverWebhookAttempt(admin,row)) delivered++;
  }
  return NextResponse.json({success:true,processed,delivered,failed:processed-delivered},{headers:{"cache-control":"no-store"}});
}
