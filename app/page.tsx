...
export default async function Home({ searchParams }: { searchParams?: Promise<{ country?: string; language?: string }> }) {
  ...
  return <div className="app-shell" lang={language} dir={isRtlLanguage(language) ? "rtl" : "ltr"}>
    <header className="topbar"><div className="container nav"><Link href="/" className="brand">GLOBAL <span>MARKET</span></Link><nav className="nav-links"><Link href={`/gold?country=${country.code}&language=${language}`}>{messages.gold}</Link><Link href={`/silver?country=${country.code}&language=${language}`}>{messages.silver}</Link><Link href={`/currencies?country=${country.code}&language=${language}`}>{language === "ar" ? "العملات" : "Currencies"}</Link><Link href={`/news?country=${country.code}&language=${language}`}>{messages.news}</Link><Link href="/pricing">{language === "ar" ? "الاشتراكات" : "Plans"}</Link></nav><div className="nav-actions"><Link className="btn btn-ghost" href="/login">{messages.login}</Link><Link className="btn btn-primary" href="/demo">{messages.createDisplay}</Link></div></div></header>
    <main>
    ...
    </main><footer className="footer"><div className="container"><div>{countries.length} countries · 4 languages</div><div style={{marginTop:8}}>{messages.referenceFooter}</div></div></footer>
  </div>;
}
