...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string; error?: string; created?: string; logged_out?: string }> }) {
  ...
  return <div className="page-frame login-page" dir="rtl" lang="ar">
    <header className="topbar"><div className="container nav site-nav">
      ...
      <details className="mobile-site-menu"><summary aria-label="فتح قائمة التنقل">☰</summary><nav aria-label="التنقل السريع"><Link href="/gold">الذهب</Link><Link href="/silver">الفضة</Link><Link href="/markets">الأسواق</Link><Link href="/stocks">الأسهم</Link><Link href="/currencies">العملات</Link><Link href="/news">الأخبار</Link><Link href="/pricing">الباقات</Link><Link href="/login">تسجيل الدخول</Link></nav></details><div className="nav-actions"><Link className="btn btn-ghost" href="/">الرئيسية</Link><Link className="btn btn-primary" href="/pricing">الباقات</Link></div>
    </div></header>
