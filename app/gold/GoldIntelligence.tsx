...
export default function GoldIntelligence({ language, countryCode, countryName, currency, gold, silver, history }: {
  ...
  return <div className="stage1-shell" dir={rtl?"rtl":"ltr"} lang={language}>
    ...
    <main className="container stage1-main">
      <section className="gold-intro"><div><div className="eyebrow"><span className="live-dot"/>ذكاء الذهب المتقدم</div><h1>الذهب كنظام بيانات، لا كسعر فقط.</h1><p>الأونصة، الغرام، العيارات، Spot/Bid/Ask، التاريخ، والتحويلات المرتبطة بمحرك تسعير واحد.</p></div><div className="gold-live-card"><span>24K / GRAM</span><strong>{money(gold.perGram24k,currency)}</strong><em>{gold.status}</em><small>{gold.provider}</small><small>مصدر: {gold.timestamp ? new Date(gold.timestamp).toLocaleString(language) : "—"} · استلام: {gold.receivedAt ? new Date(gold.receivedAt).toLocaleString(language) : "—"}</small></div></section>
