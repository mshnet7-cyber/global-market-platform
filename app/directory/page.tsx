...
export default function DirectoryPage(){
  ...
  return <div className="page-frame store-directory-page" lang="ar" dir="rtl">
    ...
    <header className="stage2-public-head"><div><div className="eyebrow"><span className="live-dot" />دليل محلات الذهب</div><h1>دليل محلات الذهب</h1><p>ملفات تجارية مرتبطة بمنصة الذهب، مع موقع وخدمات وفروع وقنوات تواصل وحالة نشر.</p></div><div><Link href="/marketplace" className="btn btn-primary">فتح السوق</Link></div></header>
    ...
    {loading?<div className="stage2-empty">جارٍ التحميل…</div>:<div className="stage2-public-grid">{rows.map(r=><article className="stage2-store-card" key={r.store_id}>
      <div className="stage2-store-mark">{(r.store?.name||"G").slice(0,1)}</div><div className="stage2-store-main"><div className="stage2-panel-head"><h2>{r.store?.name||"المحل"}</h2><span className="stage2-badge">{r.verified_at?"موثّق":"منشور"}</span></div>
    ...
    {selected&&<div className="stage2-drawer" role="dialog" aria-modal="true"><div className="stage2-drawer-panel"><button className="stage2-close" onClick={()=>setSelected(null)}>×</button><div className="eyebrow">ملف المحل</div><h2>{selected.store.name}</h2><p>{selected.directory.description||"—"}</p><div className="stage2-detail-grid"><div><span>الموقع</span><strong>{[selected.directory.address,selected.directory.city,selected.directory.region].filter(Boolean).join(" · ")||"—"}</strong></div><div><span>ساعات العمل</span><strong>{selected.directory.hours&&Object.keys(selected.directory.hours).length?"مُعرّفة":"غير مضافة"}</strong></div><div><span>الخدمات</span><strong>{(selected.directory.services||[]).join(" · ")||"—"}</strong></div><div><span>الفروع</span><strong>{(selected.branches||[]).length}</strong></div></div>
      ...
      <div className="stage2-actions"><Link className="btn btn-primary" href="/marketplace">فتح السوق</Link>{safeWebsite(selected.store.website)&&<a className="btn" href={safeWebsite(selected.store.website)!} target="_blank" rel="noreferrer">الموقع</a>}</div>
    </div></div>}
  </main>
  </div>;
}
