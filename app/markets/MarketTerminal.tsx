    ...
    <main className="container stage1-main">
      ...
      <section className="terminal-layout">
        ...
        <section className="terminal-content">
          ...
          <div className="terminal-chart-card"><div className="terminal-card-head"><div><span className="micro-label">التاريخ</span><strong>{data.history.length ? (data.history.length + " observations") : "No history"}</strong></div><div className="terminal-source">{selectedIsGold ? data.gold.provider : selectedIsSilver ? data.silver.provider : selectedQuote?.provider ?? "UNAVAILABLE"} · {selectedIsGold ? data.gold.status : selectedIsSilver ? data.silver.status : selectedQuote?.status ?? "UNAVAILABLE"}</div></div><Chart points={data.history} currency={selectedCurrency} /></div>
          <div className="terminal-grid-two">
            ...
            <section className="terminal-card"><div className="terminal-card-head"><div><span className="micro-label">مقارنة</span><strong>مقارنة سريعة</strong></div></div><div className="compare-grid"><div><span>Spot</span><b>{fmt(currentValue, selectedCurrency)}</b></div><div><span>High</span><b>{fmt(high, selectedCurrency)}</b></div><div><span>Low</span><b>{fmt(low, selectedCurrency)}</b></div><div><span>Change</span><b>{pct(derivedPercent)}</b></div></div></section>
          </div>
