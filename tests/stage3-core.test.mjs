...
test("Retry workers are scheduled in Vercel",()=>{ const v=read("vercel.json"); assert.match(v,/\/api\/cron\/webhooks/); assert.match(v,/\/api\/cron\/whatsapp/); assert.match(v,/"crons"/); });
