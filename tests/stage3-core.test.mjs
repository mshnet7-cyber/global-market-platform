...
test("OMR currency display is device-independent",()=>{const comp=read("components/MoneyDisplay.tsx");const helper=read("lib/currency-display.ts");const svg=read("public/omr-symbol.svg");assert.match(comp,/MoneyDisplayParts/);assert.match(comp,/omr-symbol/);assert.match(comp,/<svg className="omr-symbol"/);assert.match(comp,/currentColor/);assert.match(svg,/viewBox=/);assert.match(svg,/currentColor/);assert.doesNotMatch(helper,/\\u20C4|⃄/);assert.match(helper,/OMR/);});
