...
test("Billing checkout does not persist payment_pending before provider checkout succeeds",()=>{const src=read("app/api/stage3/billing/route.ts");const checkout=src.indexOf("createHostedCheckout");const persist=src.indexOf('upsert({organization_id:organization.id,plan_id:plan.id,status:"payment_pending"');assert.ok(checkout>=0);assert.ok(persist>checkout);assert.match(src,/payment_checkout_reference_missing/);assert.match(src,/subscription_checkout_persist_failed/);});
