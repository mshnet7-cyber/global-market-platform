...
test("Public marketplace order RPC is server-only",()=>{ const m=read("supabase/migrations/20260919000600_gmp_marketplace_rpc_security_hardening.sql"); assert.match(m,/revoke execute on function public\.gmp_create_marketplace_order/); assert.match(m,/from anon,authenticated/); assert.match(m,/to service_role/); });
